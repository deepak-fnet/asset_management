...
/* =========================
   WINDOWS-STYLE ANTIVIRUS DASHBOARD COMPONENT
   ========================= */
class WindowsAntivirusDashboard extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            // UI State
            activeTab: 'overview',
            installer_platform: defaultPlatform,
            ...
            // KSC Integration
            ksc_status: null,  // null | 'testing' | 'connected' | 'failed'
            ksc_error_msg: null,
            ksc_config: {
                server_url:   '',
                username:     '',
    ...
    /**
     * Waits until both chart canvases are in the DOM, then renders.
     * Retries every 100ms for up to 3 seconds to survive slow Owl patch cycles.
     * After rendering, schedules deferred resize calls so charts get correct
     * dimensions even when Odoo's action container finishes layout after mount.
     */
    async renderChartsWhenReady() {
        if (!this.chartJsLoaded) return;
        const maxRetries = 30; // 30 * 100ms = 3s
        for (let i = 0; i < maxRetries; i++) {
            const avCanvas   = document.getElementById('av_distribution_chart');
            const protCanvas = document.getElementById('av_protection_chart');
            if (avCanvas && protCanvas) {
                this.renderCharts();
                // At 150 ms just resize so charts fill their containers once
                // Odoo's action container finishes its CSS layout animation.
                setTimeout(() => {
                    Object.values(this.charts).forEach(chart => {
                        if (chart) { try { chart.resize(); } catch (e) {} }
                    });
                }, 150);
                // At 400 ms do a full re-render so the center text always
                // reflects the real state data even if the first renderCharts()
                // call ran before data was committed to state (edge-case timing).
                setTimeout(() => { this.renderCharts(); }, 400);
                return;
            }
            await new Promise(r => setTimeout(r, 100));
        }
        console.warn('⚠️ Chart canvases not found after 3s – skipping render');
    }
    ...
    async loadData() {
        ...
        try {
            // Load KPIs — Windows platform only
            const kpis = await this.orm.call("antivirus.dashboard", "get_kpis_by_platform", ['windows']);
            ...
            // Load threats — Windows platform only
            const threats = await this.orm.call("antivirus.dashboard", "get_threats_by_platform", ['windows', 10]);
            ...
            console.log("✅ Windows antivirus dashboard data loaded");

            // Re-render charts with fresh data on subsequent refreshes.
            // On the very first call (from onWillStart) the component is not
            // mounted yet, so getElementById returns null and renderCharts()
            // is a no-op — that is intentional.  onMounted → renderChartsWhenReady
            // handles the initial render with proper deferred resizes.
            if (this.chartJsLoaded) {
                await new Promise(r => setTimeout(r, 50));
                this.renderCharts();
                // Resize after refresh so charts fill their containers correctly
                setTimeout(() => {
                    Object.values(this.charts).forEach(chart => {
                        if (chart) { try { chart.resize(); } catch (e) {} }
                    });
    ...
    renderCharts() {
        ...
        // ── Helpers ──────────────────────────────────────────────────────
        // Build a per-chart center-text plugin whose total is derived from
        // the *real* data values (stored outside the dataset so Chart.js
        // internals can never shadow them).
        const makeCenterPlugin = (id, subLabel, realValues) => ({
            id,
            afterDraw(chart) {
                const { ctx, chartArea } = chart;
                if (!chartArea) return;
                // Sum the original real values, not the chart's internal data
                // array (which may contain [1] when the empty-state placeholder
                // is in use).
        ...
        // Tooltip label callback: percentage is computed against the real
        // data totals so the empty-state placeholder [1] does not inflate it.
        ...
        if (avCanvas) {
            ...
            this.charts.avDist = new window.Chart(avCanvas.getContext('2d'), {
                type: 'doughnut',
                data: {
                    labels: ['Kaspersky', 'Windows Defender', 'McAfee', 'Others'],
                    datasets: [{
                        data: hasData ? realValues : [1],
                        backgroundColor: hasData
                            ? ['#22c55e', '#3b82f6', '#ef4444', '#94a3b8']
                            : ['#e5e7eb'],
                ...
                options: {
                    ...
                    cutout: '72%',
                    plugins: {
                        // Disable the globally-registered donutCenterText plugin so it
                        // does not paint a stale "0" behind our instance-level plugin.
        ...
        const protCanvas = document.getElementById('av_protection_chart');
        if (protCanvas) {
            // get_kpis() returns protected_count / unprotected_count; fall back
            // to the initialised protected_devices / unprotected_devices keys
            // if the state hasn't been patched yet (defensive).
    ...
    setActiveTab(tab) {
        ...
        if (tab === 'devices' || tab === 'all_devices') {
            ...
            if (!this.state.threat_list || this.state.threat_list.length === 0) {
                this.loadThreats();
            }
        } else if (tab === 'deploy_center') {
            this.loadDeployCenter();
        } else if (tab === 'overview') {
            // Canvas was hidden via display:none — Chart.js loses dimensions.
            // Wait one frame for the browser to show the canvas, then resize+redraw.
            setTimeout(() => {
                Object.values(this.charts).forEach(chart => {
                    if (chart) {
                        chart.resize();
                ...
                // If charts weren't rendered yet, render them now
    ...
    async saveInstallerConfig(platform) {
        try {
            ...
            if (configId) {
                await this.orm.write("antivirus.config", [configId], {
                    [fieldName]: value
                });
            } else {
    ...
    async deployAllAntivirus() {
        try {
            // Get unprotected devices
            const unprotectedAssets = await this.orm.search("asset.asset", [
                ['antivirus_status', 'in', ['unprotected', false, null]]
            ]);

            if (!unprotectedAssets || unprotectedAssets.length === 0) {
            ...
            const confirmed = await this.showConfirmationDialog(
                `Deploy Antivirus to ${unprotectedAssets.length} Device(s)`,
                `This will deploy antivirus to ${unprotectedAssets.length} unprotected device(s). Continue?`
            );
            ...
            // Call deployment API using fetch
            const response = await fetch('/api/antivirus/deploy', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    asset_ids: unprotectedAssets,
                }),
            });
            ...
            if (result.success) {
                this.notification.add(`Deployment initiated: ${result.deployed} devices queued`, { type: "success" });
                await this.loadData();
            } else {
                this.notification.add(result.message || "Deployment failed", { type: "danger" });
            }
    ...
    async showConfirmationDialog(title, message) {
        // Simple confirmation using browser confirm (can be enhanced with Odoo dialog service)
    ...
    filterByStatus(status) {
        ...
        if (status === 'all') {
            ...
            const filtered = (this.state.threat_list || []).filter(t => t.status === status);
            this.state.threat_list = filtered;
        }
    ...
    filterBySeverity(severity) {
        ...
        if (severity === 'all') {
            ...
            const filtered = (this.state.threat_list || []).filter(t => t.severity === severity);
            this.state.threat_list = filtered;
        }
    ...
    /** Load unprotected Windows devices and KSC config from the server. */
    async loadDeployCenter() {
        try {
            const assets = await this.orm.searchRead("asset.asset",
                [["os_platform", "=", "windows"], ["antivirus_status", "in", ["unprotected", "expired"]]],
    ...
    async testKSCConnection() {
        ...
        try {
            // Odoo 19: use the framework rpc() helper instead of a hand-rolled
            // JSON-RPC envelope. It builds the envelope, forwards the session
            // context, and returns the unwrapped `result` payload.
    ...
    async saveKSCConfig(silent = false) {
        try {
            ...
            const data = await rpc('/api/antivirus/ksc/config', {
    ...
    async deployToDevice(deviceId, deviceName) {
        ...
        try {
            const data = await rpc('/api/antivirus/ksc/deploy', {
                device_ids: [deviceId],
            });
            if (!data.ok) {
    ...
    /**
     * Deploy via KSC to ALL unprotected Windows devices at once.
     */
    async deployAllUnprotected() {
        ...
        if (list.length === 0) {
            this.notification.add('No unprotected Windows devices found.', { type: 'info' });
            return;
        }
        const ok = confirm(`Deploy Kaspersky via KSC to all ${list.length} unprotected Windows device(s)?`);
        if (!ok) return;
    ...
    _pollTaskStatus(deviceId, taskId) {
        ...
        const poll = async () => {
            ...
            try {
                const data = await rpc(`/api/antivirus/ksc/status/${taskId}`, {});
    ...
    _setDeviceDeployStatus(deviceId, status, taskId, error) {
        ...
        if (idx !== -1) {
            list[idx].deploy_status = status;
            if (taskId !== null) list[idx].task_id = taskId;
            if (error !== null) list[idx].deploy_error = error;
        }
    }
    ...
    openProtectedDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "Protected Windows Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [["os_platform", "=", "windows"], ["antivirus_status", "=", "protected"]],
        });
    }
    ...
    openUnprotectedDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "Unprotected Windows Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [["os_platform", "=", "windows"], ["antivirus_status", "in", ["unprotected", "expired"]]],
        });
    }
    ...
    openKasperskyDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "Kaspersky - Windows Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [["os_platform", "=", "windows"], ["antivirus_product", "ilike", "kaspersky"]],
        });
    }

    openWindowsDefenderDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "Windows Defender Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [["os_platform", "=", "windows"], ["antivirus_product", "ilike", "defender"]],
        });
    }
    ...
    openMcAfeeDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "McAfee - Windows Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [["os_platform", "=", "windows"], ["antivirus_product", "ilike", "mcafee"]],
        });
    }
    ...
    openOtherDevices() {
        this.action.doAction({
            type: "ir.actions.act_window",
            name: "Other Antivirus - Windows Devices",
            res_model: "asset.asset",
            view_mode: "kanban,tree,form",
            views: [[false, "kanban"], [false, "list"], [false, "form"]],
            domain: [
                ...
                ["antivirus_product", "not ilike", "defender"],
    ...
    resolveDeviceImage(device) {
        // Use actual image if the asset has one
        if (device.id) {
...
/* =========================
   REGISTRY
   ========================= */
WindowsAntivirusDashboard.template = "antivirus_management.WindowsAntivirusDashboard";
registry.category("actions").add("antivirus_management.windows_dashboard", WindowsAntivirusDashboard);
