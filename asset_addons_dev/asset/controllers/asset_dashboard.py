...
class AssetDashboardController(http.Controller):
    ...
    @http.route(['/asset/dashboard/data'], type='jsonrpc', auth='user')
    def dashboard_data(self):
        ...
        dept_group = Asset._read_group([], ['department_id'], ['__count'])
        dept_labels = [dept.display_name if dept else 'Undefined' for dept, __ in dept_group]
        dept_values = [count for __, count in dept_group]
        ...
        type_group = env['history.asset']._read_group([], ['asset_type'], ['__count'])
        type_labels = [asset_type for asset_type, __ in type_group]
        type_values = [count for __, count in type_group]
        ...
        additions = Asset._read_group(
            [('acquisition_date', '>=', from_date)], ['acquisition_date:month'], ['__count'])
        month_counts_map = {
            month.strftime('%Y-%m'): count for month, count in additions if month
        }
        today = date.today()
        labels = []
        values = []
        ...
        grp = Asset._read_group([], ['plant_id', 'department_id'], ['__count'])
        # Build nested dict plant -> dept -> count
        plants = []
        plant_map = {}
        dept_set = set()
        for plant_rec, dept_rec, cnt in grp:
            plant = plant_rec.display_name if plant_rec else 'Undefined'
            dept = dept_rec.display_name if dept_rec else 'Undefined'
            dept_set.add(dept)
